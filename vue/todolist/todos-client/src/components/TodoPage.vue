        ...
        <ul class="list-group">
            <li v-for="(todo, index) in todos" class="list-group-item">
                ...
                <div class="btn-group pull-right" style="font-size:12px;line-height:1;">
                    <button type="button" class="btn-link dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">더보기
    ...
    data() {
        ...
        deleteTodo(i){
            ...
            this.todos.forEach(function(_todo,i, obj) {
                if(_todo.id === todo.id) {
                    vm.$http.delete('http://todos.garam.xyz/api/todos/'+too.id).then((result) =>{
                        obj.splice(i,1)
                    })
                }
